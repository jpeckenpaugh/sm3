...
## Derived Profile Generation — Component Composition (Not Templates)

**Correction:** earlier drafts of this specification described a Jinja-like template engine for generating derived profiles. After consultation with Vasuki and Kurma, the correct approach uses the **existing `profile_components` system** from Sprint 01 — no new template engine, no Jinja dependency.

### How it works

A derived profile like `scribe-PLAN` is a **row in the `profiles` table** with a `base_profile` foreign key pointing to `scribe`. It inherits the base profile's permissions, temperature, and header, then overlays a **mode-specific component** via `profile_components`.

```
profiles table:
  id | name        | base_profile | header (inherited) | permissions (inherited)
  ---|-------------|-------------|--------------------|-----------------------
  1  | scribe      | NULL        | {...}              | {...}
  2  | scribe-PLAN | 1 (scribe)  | (inherited)        | (inherited)
  3  | scribe-DESIGN | 1 (scribe) | (inherited)        | (inherited)

profile_components table:
  id | profile_id | component_id | order_idx | params
  ---|-----------|--------------|-----------|------
  1  | 1 (scribe) | 10 (scribe-preamble)  | 0 | {}
  2  | 1 (scribe) | 9  (scribe-domain)    | 1 | {}
  3  | 2 (scribe-PLAN) | 14 (scribe-mode-plan) | 2 | {}
```

### The inheritance algorithm

When `sm generate agent scribe-PLAN` is called:

```python
def resolve_profile(name):
    """Walk the inheritance chain and collect all components."""
    profile = get_profile_by_name(name)
    components = []
    
    if profile.base_profile:
        # Recursively collect base components first
        base_components = resolve_profile(profile.base_profile)
        components.extend(base_components)
    
    # Overlay this profile's own components
    own_components = get_components_for_profile(profile.id)
    components.extend(own_components)
    
    # Merge: child components override parent on matching component_id
    merged = merge_components_by_id(components)
    return merged
```

The mode-specific component (`scribe-mode-plan`) contains the INPUT/OUTPUT descriptions and mode instructions. Its content is assembled from the state's `file_contracts` entries at generation time — the same source of truth used by the engine at dispatch time and verification time.

### What this means for the existing system

- **`profiles` table** gains one nullable column: `base_profile TEXT REFERENCES profiles(name)`
- **`profile_components`** works exactly as it does today — no changes needed
- **`sm generate agent`** gains inheritance resolution — walk the chain, collect components, merge, render
- **`seed.py`** gains seed data for mode-specific components (one per state that has an agent)
- **No new template engine** — the existing assembly pipeline handles everything

### The mode-specific component content

A mode-specific component's `content` field contains:
...
## Inputs
  ...
  INPUT1: Concept document (concept.md)

## Outputs

  OUTPUT: Backlog feature files (backlog/ft-*.md)
  OUTPUT: Sprint brief (sprint/{:03d}/brief.md)

## Instructions

Read concept.md from INPUT1. Decompose the concept into distinct features.
Write one backlog/ft-NNN.md file per feature.
Write a sprint brief to OUTPUT describing the scope of work.

## Boundaries

- You are in PLAN mode. You do not switch modes.
- If the concept is ambiguous, escalate by writing .escalation/PLAN/<reason>.md
- Write to the exact OUTPUT paths given. No other locations.
```

The `{:03d}` placeholder is NOT resolved in the component content — it is resolved at dispatch time by the engine's `_build_request()`. The agent profile keeps the abstract form; the request text supplies the concrete path.
